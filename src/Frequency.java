...
/**
 * Convert from frequency to note, note to freq, etc.
 * 
 * @author http://www.musicdsp.org/showone.php?id=103
 */
...
public class Frequency extends Number {
	...
	private static final String NOTE_SYMBOL[] = { "C", "C#", "D", "D#", "E",
		"F", "F#", "G", "G#", "A", "A#", "B" };
	...
	public static float valueOf(String s) throws IllegalArgumentException {
		...
		try {
			...
			throw new IllegalArgumentException(
					"Neither a floating point number nor a valid note symbol.");
		}
	}
	...
	public static final int parseNoteSymbol(String s)
		...
		for (int i = NOTE_SYMBOL.length - 1; i >= 0; i--) {
			...
			try {
				return i
						+ 12
						* Integer.parseInt(s.substring(NOTE_SYMBOL[i].length())
								.trim());
			} catch (NumberFormatException _ex) {
			}
			break;
	...
	public static void transformPitch(TextComponent textcomponent, boolean flag) {
		...
		if (flag) {
			try {
				textcomponent
				.setText(Integer
						.toString((int) (getFrequency(parseNoteSymbol(s)) + 0.5F)));
				return;
			} catch (IllegalArgumentException _ex) {
				flag1 = true;
			}
		...
		try {
			textcomponent.setText(makeNoteSymbol(getPitch((new Float(s))
					.floatValue())));
			return;
		} catch (NumberFormatException _ex) {
			flag1 = true;
	...
	public Frequency(String s) throws IllegalArgumentException {
		frequency = 1.0F;
		frequency = valueOf(s);
	}
	...
	public static void main(String[] args) {
		...
		System.out.println(makeNoteSymbol(1440));
	}
}
