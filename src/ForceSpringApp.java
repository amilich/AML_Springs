...
public class ForceSpringApp extends AbstractSimulation {
	...
	int springNum = 15; //number of masses in spring
	double restLength = 10; 
	double initX = 5; 
	...
	double initLength = 10; //initial length 
	...
	boolean oscillation = true; //horizontal oscillation (on or off)
	boolean timeScale = false; //time scaling
	boolean hasGravity = true; //whether there is gravity
	boolean fix_end = true; //fixed end
	...
	protected void doStep(){
		...
		for (int ii = 0; ii < masses.size(); ii++) { //step through arraylist of masses
			masses.get(ii).color = Color.WHITE.brighter(); 
	...
	public void reset(){
		...
		control.setValue("Mouse Drag", true); 
		...
		control.setValue("Initial Length", 15); 
		...
		control.setValue("Oscillation Theta", 90); 
		control.setValue("Alpha", 0.0);
		control.setValue("G", 0); 
		control.setValue("Start Theta", 0);
	...
	public void addForces(int ii){
		...
		if(ii != springNum-1){ //the last spring does not have a downward kx
			...
			downMag = k*Math.pow(dist-restLength, 1); //calculate kx
		...
		if(ii != 0){ //the first spring does not have an upward kx 
			...
			upMag = k*Math.pow(dist-restLength, 1); //calculate kx
