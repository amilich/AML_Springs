...
public class Mass extends Circle {
	...
	double max_catch_v = 35; //maximum velocity you can catch a ball at 
	...
	boolean speedColor = false; //whether to make the mass's color dependent on its speed
	...
	public void Step(DisplayFrame frame, boolean simulated){
		...
		if(speedColor){
			if(this.velocityVector(v_x, v_y) > max_catch_v){ //deals with color change based on speed of particle  
				//change it to red (if too fast to catch) 
				super.color = Color.red; 
			}
			else {
				//change to green if you can catch it 
	...
	public void trace(DisplayFrame frame, double x, double y){
		...
		if(this.velocityVector(v_x, v_y) > max_catch_v){
			trails.get(trails.size()-1).color = Color.red; 
		}
		else {
			trails.get(trails.size()-1).color = Color.green.darker(); 
		}
